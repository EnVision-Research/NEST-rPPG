function [qrs,feats]=multi_qrsdetect_test(signal,fs,fname)
...
qrs = cell(1,4); % CHANGE ME! Using 6 levels of detection
...
%% MaxSearch (mQRS)
qrsmax=qrs{1}; % selecting as reference detector with most detections
try
    if length(qrsmax)<5
...
try
    [btype,templates,tempuncut] = beat_class(signal,qrs{2},round(WIN_ALG));
    [qrs{3},ect_qrs]=matchedQRS(signal,templates,qrs{2},btype);
...
try
    ...
    qrs{4} = consqrs(3:end-2) + lag - WIN_ALG - 1;
catch
